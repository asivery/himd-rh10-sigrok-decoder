...
std::string sj2utf8(const std::string &input)
{
  ...
  while(indexInput < input.length())
  {
    ...
    if(value == 0xFA){
      ...
      unicodeValue = 0x1100 | value;
    }else if(value == 0xFD){
      indexInput++;
      value = (uint8_t)input[indexInput++];
      unicodeValue = 0x1000 | value;
    }else{
      arraySection = ((uint8_t)input[indexInput]) >> 4;
      ...
      //unicode number is...
      unicodeValue = (convTable[arrayOffset] << 8) | convTable[arrayOffset + 1];
      goto convert;
    }

    // This is a special Sony "big character". try to find a space and remove it to offset the text
    for(int i = indexOutput-1; i>=0; i--){
      if(output[i] == ' '){
        for(int j = i + 1; j < indexOutput; j++){
          output[j - 1] = output[j];
        }
        indexOutput--;
        goto convert;
...
void Screen::clearRow(unsigned int rowNum){
  ...
  if(rowNum == this->trackBarRow){
    this->trackBar.enable = false;
  }
}
...
void I2CSSD1327Screen::begin(){
  ...
  this->disp.setFont(u8g2_font_rh10_t);
  this->disp.setFontDirection(0);
  this->disp.setBusClock(1000000);
  for(int i = 0; i<6; i++){
...
void I2CSSD1327Screen::render(){
  ...
  if(!barsEqual(&this->trackBar, &this->trackBarCache) || (this->trackBarRow != this->trackBarRowCache)){
    ...
    if(this->trackBar.enable){
      this->disp.setDrawColor(1);
      this->disp.drawRFrame(TRACK_BAR_STARTX, 16 * trackBarRow + TRACK_BAR_HMARGIN, TRACK_BAR_WIDTH, 16 - TRACK_BAR_HMARGIN, 2);
      this->disp.drawRBox(TRACK_BAR_STARTX+1, 16 * trackBarRow + TRACK_BAR_HMARGIN + this->trackBar.start, this->trackBar.end - this->trackBar.start, 16 - TRACK_BAR_HMARGIN - 1, 2);
    }
...
void I2CSSD1327Screen::renderRow(unsigned int row){
  ...
  this->disp.setCursor(0, 16 * (row + 1) - 2);
